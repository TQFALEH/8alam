...
function countHours() {
    ...
    // الساعات الحالية
    let g = parseInt(totalHours);
    
    let sumAllHours = NaN;
    sumAllHours = g + b;
    if (isNaN(b)) b = ""; if (isNaN(g)) g = "";
    document.getElementById('tHour').innerHTML = '<h6 " style="font-size: 19px; "> مجموع الساعات : ' + (g + b) + '</p>'
...
function Calgrade() {
    ...
    sumHours = countHours(sumHours);
    culmutivrGpa = sumPoints / sumHours;
    if (isNaN(culmutivrGpa) || culmutivrGpa === Infinity) culmutivrGpa = "";


    document.getElementById("semGpa").innerHTML = '<h6 id="semGpa"  style="font-size: 19px;"> معدلك الفصلي :' + semesterGpa.toFixed(2) + " " + gradeing(semesterGpa) + '</h6>';
    document.getElementById('sumPoint').innerHTML = '<h6 id="sumPoint"  style="font-size: 19px;">عدد النقاط :' + sumPoints.toFixed(2) + '</h6>';
    document.getElementById('culmGpa').innerHTML = '<h6 id="culmGpa"  style="font-size: 19px;">معدلك التراكمي:' + culmutivrGpa.toFixed(2) + " " + gradeing(culmutivrGpa) + '</h6>'
